...
def extract_text(file):
  ...
  uploaded_file_ref=client.upload_file(file=file.name)
...
def summarize(file):
  ...
  response=client.model.generate_content(
      model="gemini-2.5-flash",
      contents=[uploaded_file_ref,"Summarizethis document in simple language."]
  )
  return response.text,None
...
with gr.Blocks() as demo:
  gr.Markdown("#  🗒️ Gemini Document Analyzer")
  with gr.Row():
    ...
    with gr.Column(scale=2):
      ...
      question=gr.Textbox(
          label="Ask a specific about the document",
